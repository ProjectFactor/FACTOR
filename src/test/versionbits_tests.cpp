...
static void check_computeblockversion(const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    // Effective period: use per-deployment override if set, else global nMinerConfirmationWindow
    int nPeriod = params.vDeployments[dep].nPeriod > 0 ? params.vDeployments[dep].nPeriod : params.nMinerConfirmationWindow;
    ...
    // Check min_activation_height is on a period boundary
    BOOST_REQUIRE_EQUAL(min_activation_height % nPeriod, 0U);
    ...
    if (nTime == 0) {
        ...
        lastBlock = firstChain.Mine(nPeriod - 1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        ...
        for (uint32_t i = 1; i < nPeriod - 4; i++) {
        ...
        nTime = nStartTime;
        for (uint32_t i = nPeriod - 4; i <= nPeriod; i++) {
            lastBlock = firstChain.Mine(nPeriod + i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK_EQUAL(g_versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        }
    ...
    nTime += 600;
    uint32_t blocksToMine = nPeriod * 2; // test blocks for up to 2 time periods
    uint32_t nHeight = nPeriod * 3;
    ...
    if (nTimeout != Consensus::BIP9Deployment::NO_TIMEOUT) {
        ...
        for (uint32_t i = 0; i < nPeriod - 1; i++) {
            lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK((g_versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit)) != 0);
            nHeight += 1;
        }
