...
//
//
// ===== ASERT tests (ported from BCH node commit 1a79646efa) =====
// These require adaptation to FACTOR's uint16_t nBits before they can be uncommented.
//
...
//        blocks[i] = GetBlockIndex(&blocks[i - 1], 600, nBits);
//        BOOST_CHECK_EQUAL(
//            GetNextASERTWorkRequired(&blocks[i], &blkHeaderDummy, params, &blocks[1]),
//            nBits);
//    }
//
...
//    BOOST_CHECK(!IsASERTEnabled(params, pindexPreActivation));
...
//    BOOST_CHECK(IsASERTEnabled(params, &indexActivation0));
...
//    BOOST_CHECK(IsASERTEnabled(params, &indexActivation1));
...
//    BOOST_CHECK(IsASERTEnabled(params, &indexActivation2));
...
//    BOOST_CHECK(IsASERTEnabled(params, &indexActivation4));
